...
class CamExtractor():
    ...
    def forward_pass_on_convolutions(self, x):
        ...
        #for module_pos, module in self.model._modules.items():
            #if module_pos=='fc' : x = x.flatten(1)
            #x = module(x)  # Forward
            #if module_pos == self.target_layer:
                #conv_output = x  # Save the convolution output on that layer
        return self.output, conv_output
...
class ScoreCam():
    ...
    def generate_cam(self, input_image, device, size, is_3d= False, target_class=None):
        # Full forward pass
        ...
        #    target = torch.mean(target, 0, True)
        for i in range(len(target)):
            # Unsqueeze to 4D
            if is_3d:
                saliency_map = torch.unsqueeze(torch.unsqueeze(target[i],0),0)
                saliency_map = F.interpolate(saliency_map, size=size, mode='trilinear', align_corners=False)
            else:
                saliency_map = torch.unsqueeze(torch.unsqueeze(target[i, :, :],0),0)
            ...
            if is_3d: cam += w.data.cpu().numpy() * target[i].data.cpu().numpy()
            else: cam += w.data.cpu().numpy() * target[i, :, :].data.cpu().numpy()
