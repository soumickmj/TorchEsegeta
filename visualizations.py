...
def create_max_projection(grad, image, output_path, method_name ):
    ...
    plt.axis('off')
    plt.savefig(os.path.join(output_path, f"{method_name}_Z_prject.png"))
    ...
    plt.axis('off')
    plt.savefig(os.path.join(output_path, f"{method_name}_Z_prject_overlay1.png"))

    # Saving attribution with overlay z project -- Tyoe1
    plt.imshow(z_proj_input.transpose(), cmap='Greys')
    plt.imshow(result.transpose(), cmap='Reds', alpha=0.7)
    plt.axis('off')
    plt.savefig(os.path.join(output_path, f"{method_name}_Z_prject_overlay2.png"))
